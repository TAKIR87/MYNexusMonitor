...
КОММИТ:
   ...
   Инцидент №6: Несоответствие порта API во фронтенде
──────────────────────────────────
СИМПТОМ:
   Фронтенд пытается обратиться к API по адресу http://localhost:8001/api/incidents,
   но бэкенд запущен на порту 8000. В консоли ошибка: ERR_CONNECTION_REFUSED.

ПРИЧИНА:
   В настройках фронтенда задан порт 8001, не совпадающий с реальным портом бэкенда.

РЕШЕНИЕ:
   В переменной окружения VITE_API_URL (или аналогичной) порт исправлен на 8000.

КОММИТ:
   fix(api): correct API port from 8001 to 8000 in frontend configuration